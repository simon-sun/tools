<?php
class Banks{
    protected static  $cardType = ['CC' => '信用卡', 'DC' => '储蓄卡'];
    protected static  $bankInfo = [
        "SRCB"      => "深圳农村商业银行",
        "BGB"       => "广西北部湾银行",
        "SHRCB"     => "上海农村商业银行",
        "BJBANK"    => "北京银行",
    ...
    public static function curl_get($url){
        ...
        curl_setopt($ch, CURLOPT_USERAGENT, 'Mozilla/5.0 (Linux; U; en-us; KFAPWI Build/JDQ39) AppleWebKit/535.19 (KHTML, like Gecko) Silk/3.13 Safari/535.19 Silk-Accelerated=true'); // 伪造User-Agent
//        curl_setopt($ch, CURLOPT_USERAGENT, 'Mozilla/5.0 (compatible; MSIE 10.0; Windows NT 6.2; Trident/6.0)'); // 伪造User-Agent
        curl_setopt($ch, CURLOPT_HTTPHEADER , array('CLIENT-IP:110.76.6.93','X-FORWARDED-FOR:110.76.6.93','HTTP_HOST:'.$host['host'],'X-FORWARDED-HOST:'.$host['host'], 'X-FORWARDED-SERVER:'.$host['host'])); // 伪造HTTP头
