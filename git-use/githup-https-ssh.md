...
使用ssh：
git clone https://github.com/simon-sun/tools.git
使用https
git clone git@github.com:simon-sun/tools.git
...
git status 查看状态
git add --all 添加所有更新文件
git add index.html 添加一个文件
git commit -m "备注信息" 放入暂存区
git push 刷新到正式线上 （如果是https 需要输入账号的用户名和密码，如果是ssh直接更新）
